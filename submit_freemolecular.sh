...
srun ./pbIterativeSolver -cells 100 -length 1 -u 1 -k $KERNEL -p 16 -res 1e-12 -mass -nin mono &> "$KERNEL"_p16_res1e-12_delta_cells100_length1_u1_mf.log

#srun ./pbSolve -t 12 -N 16384 -L 1 -alpha 0.1 -k continuum &> data/continuum_N16384_L1_alpha0.1_t12.log

echo "" >> "$KERNEL"_p16_res1e-12_delta_cells100_length1_u1_mf.log

# N.B. Can use sacct, in order to find lots of information about CPU times, memory use and disk access, using for example
sacct --job $SLURM_JOBID --format "JobName,Submit,Elapsed,AveCPU,CPUTime,UserCPU,TotalCPU,NodeList,NTasks,AveDiskRead,AveDiskWrite" >> "$KERNEL"_p16_res1e-12_delta_cells100_length1_u1_mf.log
